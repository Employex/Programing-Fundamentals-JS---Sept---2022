function adAstra(input) {
  let pattern =
    /([|#])(?<name>[A-Za-z ]+)\1(?<date>\d{2}\/\d{2}\/\d{2})\1(?<kcal>\d+)\1/gm;
  ...
  while (exec) {
    ...
    currentActions = `Item: ${name}, Best before: ${date}, Nutrition: ${kcal}`;
    productStore.push(currentActions);

    exec = pattern.exec(input);
  }
  let days = Math.floor(totalKcal / 2000);
  console.log(`You have food to last you for: ${days} days!`);
  productStore.forEach((element) => {
    console.log(element);
  });
}
adAstra([
